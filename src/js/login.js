...
export default class Login {
  constructor() {
    ...
    if (caching) {
      ...
      cachedUser.nickname =
        localStorage.getItem(LOGIN_CACHE_KEY_NICKNAME) || "";

      /// advanced option
      cachedAdvanced.active =
        localStorage.getItem(LOGIN_CACHE_KEY_ADVANCED) === "true" || false;
      cachedAdvanced.wsHost =
        localStorage.getItem(LOGIN_CACHE_KEY_ADVANCED_WS_HOST) || "";
      cachedAdvanced.apiHost =
        localStorage.getItem(LOGIN_CACHE_KEY_ADVANCED_API_HOST) || "";
      cachedAdvanced.deskApiHost =
        localStorage.getItem(LOGIN_CACHE_KEY_ADVANCED_DESK_API_HOST) || "";
    }
    ...
    const nickname = simplify(
      this.element.querySelector(".form-login .nickname")
    );

    let advancedActive = cachedAdvanced.active;
    const advancedLabel = simplify(
      this.element.querySelector(".advanced .label")
    );
    ...
    const wsHost = simplify(
      this.element.querySelector(".form-advanced .wsHost")
    );
    const apiHost = simplify(
      this.element.querySelector(".form-advanced .apiHost")
    );
    const deskApiHost = simplify(
      this.element.querySelector(".form-advanced .deskApiHost")
    );
    if (!ADVANCED_OPTION) {
      advancedLabel.hide();
    }
        ...
        if (caching) {
          ...
          if (advancedActive) {
            ...
            localStorage.setItem(
              LOGIN_CACHE_KEY_ADVANCED_WS_HOST,
              options.wsHost
            );
            localStorage.setItem(
              LOGIN_CACHE_KEY_ADVANCED_API_HOST,
              options.apiHost
            );
            localStorage.setItem(
              LOGIN_CACHE_KEY_ADVANCED_DESK_API_HOST,
              options.deskApiHost
            );
          } else {
            options.wsHost = null;
