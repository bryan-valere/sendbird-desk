...
export default class Widget {
  constructor(user, options) {
    ...
    let tabs = simplify(
      document.querySelectorAll(".-sbd-tabs > .-sbd-tab-item")
    );
    ...
    let menus = simplify(
      document.querySelectorAll(".-sbd-menu-list > .-sbd-menu-item")
    );
    ...
    this.ticketList = simplify(
      document.querySelector(".-sbd-panel > .-sbd-ticket-list")
    );
    ...
    let ticketNew = simplify(
      document.querySelector(".-sbd-ticket-list > .-sbd-ticket-new")
    );
    ticketNew.on("click", () => {
     ...
     */
    const accessToken = "f8c6c1060ecebe14091b3fedc9c1d258d0d3279d";
            ...
            (res, err) => {
              ...
              channelHandler.onChannelChanged = (channel) => {
                SendBirdDesk.Ticket.getByChannelUrl(channel.url, (res, err) => {
                  ...
                  let ticketElement =
                    ticketElementIndex >= 0
                      ? this.ticketElementList[ticketElementIndex]
                      : null;
                  if (ticketElement) {
                  ...
                  const isInitial =
                    ticket.status === SendBirdDesk.Ticket.Status.INITIALIZED;
                  const isClosed =
                    ticket.status === SendBirdDesk.Ticket.Status.CLOSED;
                  const isNewest = ticketElementIndex === 0;
                  if (
                    this.currentTab !== SendBirdDesk.Ticket.Status.CLOSED &&
                    !isNewest &&
                    !isClosed
                  ) {
                    ...
                    if (ticketElement) {
                      if (this.ticketElementList.length > 0) {
                        this.ticketList.insertBefore(
                          ticketElement.element,
                          this.ticketElementList[0].element
                        );
              ...
              channelHandler.onMessageReceived = (channel, message) => {
                if (
                  !this.currentTab ||
                  this.currentTab === SendBirdDesk.Ticket.Status.OPEN
                ) {
                  let data = null;
                  try {
                  ...
                  SendBirdDesk.Ticket.getByChannelUrl(
                    channel.url,
                    (res, err) => {
                      if (err) throw err;
                      const ticket = res;
                      ...
                      message.isAssigned =
                        data &&
                        data.type ===
                          SendBirdDesk.Message.DataType.TICKET_ASSIGN;
                      message.isTransferred =
                        data &&
                        data.type ===
                          SendBirdDesk.Message.DataType.TICKET_TRANSFER;
                      message.isClosed =
                        data &&
                        data.type ===
                      ...
                      const ticketElementIndex =
                        this.ticketElementList.findIndex(
                          (ticketElement) =>
                            ticketElement.ticket.id === ticket.id
                        );
                      const ticketElement =
                        ticketElementIndex >= 0
                          ? this.ticketElementList[ticketElementIndex]
                  ...
                  SendBirdDesk.Ticket.getByChannelUrl(
                    ...
                    (res, err) => {
                      ...
                      message.isClosureInquired =
                        data &&
                        data.type ===
                          SendBirdDesk.Message.DataType.TICKET_INQUIRE_CLOSURE;
                      if (message.isClosureInquired) {
                        ...
                        switch (closureInquiry.state) {
                          case SendBirdDesk.Message.ClosureState.CONFIRMED:
                            ticket.status = SendBirdDesk.Ticket.Status.CLOSED;
                            if (ticketElement) {
                              this.ticketElementList.splice(
                                ticketElementIndex,
                                1
                              );
                              this.ticketList.removeChild(
                                ticketElement.element
                              );
                            }
                            break;
  ...
  isBottom() {
    return (
      this.ticketList.scrollHeight - this.ticketList.scrollTop ===
      this.ticketList.clientHeight
    );
  }
